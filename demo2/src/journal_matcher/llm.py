    ...
    def complete_json(self, prompt: str) -> Any:
        ...
        if "recommend suitable mathematics journals" in lower:
            ...
            def relevance(candidate):
                ...
                score = 0
                if "knot" in name or "legendrian" in name:
                    score += 8
                if "symplectic" in name or "geometric mechanics" in name:
                    score += 6
                if "geometry" in name or "topology" in name:
                    score += 3
                if "survey" in name or "bulletin" in name:
                    score += 1
                return score
            ...
            for index, candidate in enumerate(selected):
                ...
                recommendations.append({"role": roles[index], "journal_id": journal_id, "journal_name": name, "fit": "strong match" if index == 0 else "plausible match", "reasons": ["The catalog contains recent publications in the confirmed MSC area.", "The journal title and field tags are compatible with the article's topology/symplectic focus."], "level_fit": "closely aligned" if index == 0 else "insufficient information", "representative_article_ids": representatives[:3], "important_mismatch": "Offline smoke-test model does not inspect the full journal scope or acceptance policies.", "submission_emphasis": "Explain the classification theorem, explicit Legendrian constructions, and implications for fillings and augmentation varieties."})
